...
lock directory) that the next run trips on for up to a minute — the
harness treats its refresh lock as stale after 60 s (`stale: 60000`,
read from its binary), so it clears itself, but only then.
