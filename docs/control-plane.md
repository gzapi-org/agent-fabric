...
is refused when dated more than a minute ahead of the account's clock (a
fast operator clock would otherwise raise the replay floor, below, past
every honest action), and is accepted only when strictly newer than the last
action accepted from its sender — a ledger persisted in the account's fabric state
