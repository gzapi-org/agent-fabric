...
- **the marker** — `runtime/claude-code/hooks/plan-hold.sh`, on every
  ...
  and `SessionEnd`, writes `/tmp/agent-fabric-hold-<uid>/<login>.json`
  while the event's `permission_mode` is `plan` and clears it otherwise.
  The marker names the session and the harness pid (`CLAUDE_PID`, which
  hooks receive; verified live on 2.1.273). It follows the mode, not the
  ...
  ignored.
- **the watch** — checks the marker before every slice and once a
  second during one. Held: it polls nothing (an in-flight long-poll is
  cut; nothing was acknowledged, so the relay re-shows it). Released: it
  polls, and whatever accumulated is delivered in one page. Held and
  released are one line each on stderr, never stdout — stdout is what
...
**Staleness.** A marker is a hold only while the pid it names is alive.
A session that dies planning leaves a marker the next hook event of any
session under that login removes, and the watch ignores meanwhile; a
reboot empties `/tmp`. Without `jq` the hook writes nothing and the
inbox behaves as before this change.

**Scope.** Per address, like the cursor: two sessions under one login
share the hold as they share the cursor. A sender with `REPLY-EXPECTED:
