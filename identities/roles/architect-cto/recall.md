...
role: architect-cto
class: recall
description: "Where architect-cto's knowledge lives — charter, remit, distilled slices, this agent's memory — and how to trace a claim to its sources."
tier: 2
...
- **The charter** (`identities/roles/architect-cto/charter.md`, here) — what the
  function is. The project's remit for it
  (`<working copy>/.agent-fabric/roles/architect-cto.md`) — what the function
  covers in that repository.
- **The distilled slices** — `memory/domains/architect-cto/` here for the field,
  `<working copy>/.agent-fabric/memory/architect-cto/` in the project for the
