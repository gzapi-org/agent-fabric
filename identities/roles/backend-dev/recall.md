...
role: backend-dev
class: recall
description: "Where backend-dev's knowledge lives — charter, remit, distilled slices, this agent's memory — and how to trace a claim to its sources."
tier: 2
...
- **The charter** (`identities/roles/backend-dev/charter.md`, here) — what the
  function is. The project's remit for it
  (`<working copy>/.agent-fabric/roles/backend-dev.md`) — what the function
  covers in that repository.
- **The distilled slices** — `memory/domains/backend-dev/` here for the field,
  `<working copy>/.agent-fabric/memory/backend-dev/` in the project for the
