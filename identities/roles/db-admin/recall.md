...
role: db-admin
class: recall
description: "Where db-admin's knowledge lives — charter, remit, distilled slices, this agent's memory — and how to trace a claim to its sources."
tier: 2
...
- **The charter** (`identities/roles/db-admin/charter.md`, here) — what the
  function is. The project's remit for it
  (`<working copy>/.agent-fabric/roles/db-admin.md`) — what the function
  covers in that repository.
- **The distilled slices** — `memory/domains/db-admin/` here for the field,
  `<working copy>/.agent-fabric/memory/db-admin/` in the project for the
