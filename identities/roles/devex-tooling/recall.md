...
role: devex-tooling
class: recall
description: "Where devex-tooling's knowledge lives — charter, remit, distilled slices, this agent's memory — and how to trace a claim to its sources."
tier: 2
...
- **The charter** (`identities/roles/devex-tooling/charter.md`, here) — what the
  function is. The project's remit for it
  (`<working copy>/.agent-fabric/roles/devex-tooling.md`) — what the function
  covers in that repository.
- **The distilled slices** — `memory/domains/devex-tooling/` here for the field,
  `<working copy>/.agent-fabric/memory/devex-tooling/` in the project for the
