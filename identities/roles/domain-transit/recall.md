...
role: domain-transit
class: recall
description: "Where domain-transit's knowledge lives — charter, remit, distilled slices, this agent's memory — and how to trace a claim to its sources."
tier: 2
...
- **The charter** (`identities/roles/domain-transit/charter.md`, here) — what the
  function is. The project's remit for it
  (`<working copy>/.agent-fabric/roles/domain-transit.md`) — what the function
  covers in that repository.
- **The distilled slices** — `memory/domains/domain-transit/` here for the field,
  `<working copy>/.agent-fabric/memory/domain-transit/` in the project for the
