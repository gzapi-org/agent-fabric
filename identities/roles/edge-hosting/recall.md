...
role: edge-hosting
class: recall
description: "Where edge-hosting's knowledge lives — charter, remit, distilled slices, this agent's memory — and how to trace a claim to its sources."
tier: 2
...
- **The charter** (`identities/roles/edge-hosting/charter.md`, here) — what the
  function is. The project's remit for it
  (`<working copy>/.agent-fabric/roles/edge-hosting.md`) — what the function
  covers in that repository.
- **The distilled slices** — `memory/domains/edge-hosting/` here for the field,
  `<working copy>/.agent-fabric/memory/edge-hosting/` in the project for the
