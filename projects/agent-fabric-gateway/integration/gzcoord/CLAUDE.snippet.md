...
agent-fabric under `communication/gzcoord/`; the gateway's integration — the
relay, the channel, the session-start drain — is
`projects/agent-fabric-gateway/integration/gzcoord/` there, on the same relay and
...
- as `ROLE`, the slug of the role you hold (your launch prompt says it; `identities/roles/catalog.json`) — `fabric-coordinator`, never a title; `fabric-whoami --json` reports your address, role and project;
