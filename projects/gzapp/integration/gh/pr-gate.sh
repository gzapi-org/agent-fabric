#!/usr/bin/env bash
# gzapp's forwarder to the fabric's runtime/github/pr-gate.sh — the one place
...
# gzapp's tools/gh/pr-gate.sh forwards here, argv and stdin untouched.
#
...
exec bash "$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)/runtime/github/pr-gate.sh" "$@"
