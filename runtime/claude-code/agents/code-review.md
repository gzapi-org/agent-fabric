---
name: code-review
description: "The REVIEW CLASS (capability code-review): the adversarial blind review of a PR — THE review, no automated reviewer ahead of it: dispatched on every PR's head, to judge a finding before it is answered (step 26 of the canonical lifecycle), and to re-review a fix range; posted with runtime/github/post-review.sh, which the gate counts. Briefed by bin/fabric-review: facts, never conclusions. Deliberately blind: given a repository path and a base..head range, never the reasoning behind the change. Standing authorisation for the class — model fable, no per-dispatch ask, no worktree (the tree is read-only for it), description beginning review or re-review; the dispatch guard denies any other shape. The dispatch says `fable` (the tier alias; the Agent tool accepts only the four aliases) and, under a fabric launch, the guard drops it so the `model:` line of this file decides — install-agent-files.sh writes the model routing resolves for code-review there, for the launch's provider (routing/capabilities.json and the profile layers, gated by routing/policies/review-grade.json: claude-opus-5[1m] on plain claude, DeepSeek V4 Pro on the broker — the top tier of each path). Never the fable export: code-plan rides fable too, and through the export the reviewer would follow it, as it once followed code-high on opus (live, 2026-09-13). Unlaunched, the alias binds to the harness's current Fable tier."
model: fable
