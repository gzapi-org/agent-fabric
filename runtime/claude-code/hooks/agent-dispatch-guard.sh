...
jq -c '
  ...
  | if $type == "fork" then empty
      ...
      if ($review_desc | not) then
        ...
        deny("Review dispatch with model \"" + ($t.model // "unset") + "\". The review class runs opus, always: a review is not a retryable step, its failure mode is a green PR that merges, so no dropping to a cheaper tier because the review looks small. Set model: opus. That pin is the harness TIER alias, not a vendor model id -- the concrete target is resolved at LAUNCH by tools/launch/ori from .roles/registry/model-profiles.json and validated against review_grade, so policy stays here and vendor plumbing stays in the launch profile. See CLAUDE.md - Subagent dispatch.")
      elif $iso != "" then
