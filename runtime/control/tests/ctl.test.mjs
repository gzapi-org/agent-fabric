...
test('presence: one row per account — running since when, as what; none; a failed read says unknown', () => {
  ...
  const planning = table('presence', rows([expected[0]], [reply('h/web-dev-01', { status: 'ok', online: true, sessions: 1, since: '2026-09-25T09:57:22.000Z', role: 'web-dev', project: 'gzapp', planning: true })])).split('\n');
  assert.match(planning[1], /^web-dev-01\s+planning\s+2026-09-25/, 'a session that is planning says so');
