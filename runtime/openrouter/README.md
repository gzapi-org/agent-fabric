...
| refusals | none | no bound role, model pins in any settings scope, an ungraded review pin, a non-Anthropic session, no long-lived Claude sign-in (docs/claude-accounts.md) | the same, plus `ori` not authenticated from the environment |
