...
grep -q "CLAUDE-ENV:TMPDIR=$HOME/.cache/tmp" <<<"$out" && [[ -d "$HOME/.cache/tmp" ]] && ok "TMPDIR under the home for every session, created" || bad "TMPDIR not exported under the home" "$out"
