...
| `OPENROUTER_API_KEY`, `GH_TOKEN`, `CLAUDE_BRIDGE_AUTH_TOKEN`, `SERPER_API_KEY`, `BRAVE_SEARCH_API_KEY` (language-culture logins: the locale search tools, `runtime/mcp/websearch-locale`) | exported from `~/.config/agent-fabric/secrets.env` (0600), sourced by `~/.bashrc` |
