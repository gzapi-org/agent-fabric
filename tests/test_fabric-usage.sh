...
out="$(HOME="$H" PATH="$SANDBOX/bin:$PATH" sh -c "$READ" 2>&1)"
[[ "$out" != "setup-token" ]] && ok "an empty synced value is no template, as syncedVar reads it" || bad "empty value read as a template" "$out"
rm -f "$H/.config/agent-fabric/secrets.env" "$SANDBOX/curl-ran"
out="$(HOME="$H" PATH="$SANDBOX/bin:$PATH" sh -c "$READ" 2>&1)"
