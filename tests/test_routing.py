...
GLM_SHIM = "@preset/glm-claude-compat"
...
def test_current_glm_policy() -> None:
    expected = {
        "code-low": ("z-ai/glm-5.3-flash", GLM_SHIM, "z-ai/glm-5.3-flash@preset/glm-claude-compat"),
        "code-medium": ("z-ai/glm-5.2", GLM_SHIM, "z-ai/glm-5.2@preset/glm-claude-compat"),
        "code-high": ("z-ai/glm-5.3", GLM_SHIM, "z-ai/glm-5.3@preset/glm-claude-compat"),
...
def test_composite_is_derived_not_stored() -> None:
    for name in ("capabilities.json", "profiles.json", "shims.json"):
        ...
        assert "@preset/glm-claude-compat" not in text or name == "shims.json", \
...
def test_a_non_glm_model_gets_no_shim(tmp: str) -> None:
    ...
    assert res["composite"] == "z-ai/glm-5.2@preset/glm-claude-compat", res
    # And the real files are untouched.
    assert routing.resolve("code-high", "openrouter")["composite"] == "z-ai/glm-5.3@preset/glm-claude-compat"
...
def test_shim_follows_the_family_of_the_merged_model(tmp: str) -> None:
    ...
    assert routing.resolve("code-low", "openrouter", local=local, root=root)["composite"] == "z-ai/glm-5.2@preset/glm-claude-compat"
...
def test_check_refuses_a_preset_as_a_model(tmp: str) -> None:
    root = scratch_root(tmp)
    set_model(root, "openrouter", "code-low", "@preset/glm-claude-compat")
    findings = routing.check(root)
