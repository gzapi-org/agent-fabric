...
Both are optional to the core protocol. A deployment that numbers its messages — as a lossy transport makes necessary, since only a complete per-sender sequence can be reconciled against — SHOULD have its validator warn when `MESSAGE-ID` is absent, and when an unknown key reads as a misspelling of a common field or carries an id-shaped value without being an id field. Warnings, not errors: §6 requires unknown metadata to be preserved, because that is how the protocol extends, and a validator that rejected a key it did not recognise would close the extension path it is required to keep open.
